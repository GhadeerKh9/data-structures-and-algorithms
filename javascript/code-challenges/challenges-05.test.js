...
const addValues = (arr) => {
  let locArr = arr.reduce((acc, item, idx) => {
    acc = acc + item;
    return acc;
  }, 0);
  return locArr;
...
const countNumberOfElements = (arr) => {
  let locArr = arr.reduce((acc, val, idx) => {
    acc = acc + 1;
...
const reversedString = (str) => {
  let locArr = str.split("");
  let reversedString = locArr.reduce((acc, item, idx) => {
    acc = item + acc;
    return acc;
  }, "");
