...
function transformToLis(obj) {
  let locArr = Object.entries(obj).map((entry) => {
...
const count = (target, input) => {
  ...
  let secArr = locArr.filter((value) => value === target);
...
let findMaleAndFemale = (data) => {
  let locArr = data.filter((obj) => {
    if (obj.gender === "male" || obj.gender === "female") {
      return obj;
    }
  });

  let names = locArr.map((item) => {
    return item.name;
  });
...
let findShortest = (data) => {
  let locArr = data.sort((a, b) => a.height - b.height);

  return locArr[0].name;
};
